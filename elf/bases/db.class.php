...
abstract class DB {
	...
	function insert($array=null, $filter=1, $validate=0){
		...
		if($validate){
			$vali=Validate::check($array, "add", $this,$this->databaseconf[$this->config[0]][$this->config[1]]['tableprefix']);
		}else{
			$vali=true;	
		}
	...
	function update($array=null, $filter=1, $validate=0){
		...
		if($validate){
			$vali=Validate::check($array, "mod", $this,$this->databaseconf[$this->config[0]][$this->config[1]]['tableprefix']);
		}else{
			$vali=true;	
		}
